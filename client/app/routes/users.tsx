...
const UserTable = React.lazy(async () => ({
  default: (await import("~/components/users/UserTable")).UserTable,
}));
...
/**
 * Loader function to fetch users from the API.
...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  const { data: users, ...pagination } = await userService.getUsersWithRoles(
    page,
    filters
  );
